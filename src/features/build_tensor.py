...
Feature channel groups
  ...
  G3  weather       NOAA TMAX, TMIN, PRCP, GDD (shared)           shape (N, T, 4)
  G4  static        one-hot crop attributes (time-invariant)      shape (N, T, D)

Leakage rules
  - Fit all scalers on the training slice only, then apply to train and test.
  ...
  - Call build_feature_groups() inside each rolling-origin fold with
    panel_train and weather_train; then apply the same scalers to the test slice.

Ablation
Pass include=['G0','G1','G2'] to concat_groups() to drop groups.
That is how the feature-group ablation in Phase 6 works.
'''
...
def fit_crop_scalers(panel_train: pd.DataFrame) -> dict:
    ...
    Returns {crop: fitted_scaler}. Store this with the fold and reuse it
    on the test slice via transform_crop_scalers().
    '''
    scalers = {}
...
def transform_crop_scalers(panel: pd.DataFrame,
    ...
    '''Apply pre-fitted crop scalers to a train or test slice.'''
    out = panel.copy()
...
def fit_weather_scaler(weather_train: pd.DataFrame) -> RobustScaler:
    '''Fit a RobustScaler on training weather rows (NaN rows dropped first).'''
    scaler = RobustScaler()
...
def build_feature_groups(panel:         pd.DataFrame,
    ...
    Parameters
    panel          : (T, N) Trends panel slice; columns should cover crops.
    weather        : (T, 4) NOAA weather slice, aligned to panel.index.
    crops          : ordered list of N crop names.
    crop_scalers   : from fit_crop_scalers() on the train slice.
                     Pass None to skip scaling (e.g. quick EDA).
    weather_scaler : from fit_weather_scaler() on the train slice.
    groups         : group keys to build; default is ALL_GROUPS.

    Returns
    dict of group_key -> np.ndarray with shape (N, T, C_g)
    '''
    if groups is None:
        groups = ALL_GROUPS
    ...
    if 'G0' in groups:
        ...
        # Per crop: [value, lag1, lag2, lag4, lag8, lag13, lag26, lag52]
        g0 = np.stack([
            pd.concat([p[[c]],
    ...
    # G4: static node attributes (time-invariant, broadcast over time)
    if 'G4' in groups:
...
def concat_groups(feature_groups: dict,
    ...
    include        : keys to concat; default is all present keys.
...
def channel_index(feature_groups: dict, include: list = None) -> list:
    '''Return ordered (group, channel_idx) tuples for interpretability.'''
    keys = include if include is not None else list(feature_groups.keys())
    index = []
    for k in keys:
...
def build_from_files(crops: list = None,
    ...
    '''Load panel + weather from config paths and build a feature tensor.

    Returns (X, crops, panel_index, feature_groups) where X has shape
    (N, T, C_total).

    Note: this does not apply scalers. Use it for EDA only.
