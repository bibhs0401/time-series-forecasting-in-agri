...
def a0_integrity(anomalies: list) -> pd.DataFrame:
    ...
    for _, r in df.iterrows():
        group, seam = r['group'], r['seam']
        if r['type'] == 'empty_window':
            print(f'      {group} {seam}: EMPTY window file '
                  f'(re-download this pull)')
        else:
            n_overlap = r['n_overlap']
            print(f'      {group} {seam}: appended UNSCALED '
                  f'(only {n_overlap} overlap wk) — calibration break')
    print('  → wrote a0_seam_integrity.csv')
    return df
...
def a1_overlap_error(seam_records: list) -> pd.DataFrame:
    ...
    n_groups = by_seam['group'].nunique()
    ...
    if len(flagged):
        for _, r in flagged.sort_values('median_abs_rel', ascending=False).iterrows():
            group, seam = r['group'], r['seam']
            median_abs_rel = r['median_abs_rel']
            n_overlap = int(r['n_overlap'])
            print(f'      {group:>4} {seam}  '
                  f'median {median_abs_rel:.1%}  '
                  f'({n_overlap} wks)')
    print('  → wrote a1_seam_residuals.csv, a1_seam_summary.csv')
...
def a2_anchor_loo(stitched: dict) -> pd.DataFrame:
    ...
    *every* crop it shares with the reference group, not just the configured
    ...
    for g in config.GROUPS:
        ...
        anchor_names = ', '.join(factors.keys())
        print(f'  {g:>4}: {n_anchors} anchor(s) [{anchor_names}]  '
    ...
    if len(multi):
        ...
        worst_ratio = worst['max_min_ratio']
        print(f'  Largest anchor disagreement:  {worst_group} '
              f'(max/min ratio {worst_ratio:.3f})')
    print(f'  Groups anchored on cucumber ALONE: {n_single}  '
          f'(only direct g1 bridge is tomato in g3)')
    print('  → wrote a2_anchor_factors.csv, a2_anchor_summary.csv')
    return summ
...
def a3_estimator_sensitivity() -> pd.DataFrame:
    ...
    if len(per_crop_df):
        ...
        crop = w['crop']
        median_dev = w['median_abs_rel_dev']
        print(f'  Most-affected crop:     {crop} '
              f'(median dev {median_dev:.1%})')
    print('  → wrote a3_estimator_sensitivity.csv')
    return per_crop_df
...
def a5_noise_floor(seam_records: list) -> pd.DataFrame:
    ...
    print('  Per-crop floor (median abs overlap disagreement):')
    panel_median = floor['noise_floor_med_abs_rel'].median()
    print(f'    panel-wide median:  {panel_median:.2%}')
    print('    noisiest crops:')
    for _, r in floor.head(6).iterrows():
        crop = r['crop']
        noise_floor = r['noise_floor_med_abs_rel']
        print(f'      {crop:<22} {noise_floor:6.2%}')
    print('\n  NOTE: this is a *lower bound* (one pull/window, no replicates).')
    print('        For a true floor, collect 3× repeat pulls on a few cells.')
    print('  → wrote a5_noise_floor.csv')
    return floor
...
def main():
    ...
    print('  (A4 Wikipedia corroboration: run '
